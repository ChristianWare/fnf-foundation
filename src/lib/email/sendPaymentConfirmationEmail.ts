...
export async function sendPaymentConfirmationEmail(
  ...
  const subject = isDepositReceipt
    ? `✅ Deposit Received – Balance of ${formatMoney(args.balanceCents ?? 0, args.currency)} Due${args.balanceDueDate ? ` by ${args.balanceDueDate}` : ""} | ${SITE.name}`
    : `🎉 Payment Confirmed – Your Ride is Booked | ${SITE.name}`;
        ...
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width: 560px; background-color: ${colors.white}; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 24px rgba(0, 0, 0, 0.08);">
          ...
          <tr>
            <td style="background-color: ${colors.black}; padding: 28px 32px; text-align: center;">
              <h1 style="margin: 0; color: ${colors.white}; font-size: 22px; font-weight: 600; letter-spacing: -0.5px;">${SITE.name.toUpperCase()}</h1>
            </td>
          </tr>
          ...
          <tr>
            <td style="background-color: ${colors.cream}; padding: 24px 32px; text-align: center; border-top: 1px solid ${colors.stroke};">
              ...
              <p style="margin: 0; color: ${colors.paragraph}; font-size: 12px; opacity: 0.7;">© ${new Date().getFullYear()} ${SITE.name}. All rights reserved.</p>
            </td>
          </tr>
...
export async function buildInvoiceDataForBooking(bookingId: string): Promise<{
  ...
  const company = {
    name: companySettings.officeName || SITE.name,
