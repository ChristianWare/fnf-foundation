...
export async function sendBookingDeclinedEmail(args: {
  ...
  const contactEmail = args.contactEmail || SITE.email.support;
  const contactPhone = args.contactPhone || null;

  const subject = `Booking Update – Unable to Accommodate Your Request | ${SITE.name}`;
...
<body style="margin: 0; padding: 0; background-color: ${colors.cream}; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;">
          ...
          <tr>
            <td style="padding: 24px 32px 0 32px; text-align: center;">
              ...
              <p style="margin: 0; color: ${colors.paragraph}; font-size: 16px; line-height: 1.6;">
                Thank you for considering ${SITE.name}. Unfortunately, we are unable to accommodate your booking request at this time.
              </p>
            </td>
          </tr>
  ...
  const text = [
    ...
    `Hi ${firstName},`,
    "",
    `Thank you for considering ${SITE.name}. Unfortunately, we are`,
