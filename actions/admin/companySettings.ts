...
// Fallbacks come from src/config/site.ts so a fresh deploy already shows the
// client's name, phone, and email before anyone opens Admin → Company Settings.
const usDigits = (tel: string) => {
...
const DEFAULTS = {
  ...
  dispatchPhoneRaw: usDigits(SITE.phone.tel),
  emergencyPhone: SITE.phone.display,
  emergencyPhoneRaw: usDigits(SITE.phone.tel),
  ...
  websiteUrl: SITE.url,
  googleBusinessUrl: SITE.social.google,
